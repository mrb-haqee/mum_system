...
if (!$dataCekUser || !$dataCekMenu || !validateIP($_SESSION['IP_ADDR']) || !$tokenValid) {
    ...
    try {
        ...
        //     if ($status) {
        //         $pesan = 'Proses Update Budgeting Project Berhasil';
        //     } else {
        //         $pesan = 'Proses Update Budgeting Project Gagal';
        //     }
        ...
        if ($flag === 'tambah') {
            ...
            if ($status) {
                updateNomorUrut($db, 'budgeting_project_progres', $idUserAsli);
                $pesan = 'Proses Tambah Budgeting Project Berhasil';
            } else {
                $pesan = 'Proses Tambah Budgeting Project Gagal';
            }
            ...
            $status = statementWrapper(
                DML_UPDATE,
                "UPDATE budgeting_project_progres SET statusBudgetingProjectProgres = ? WHERE idBudgetingProjectProgres = ?",
                ['Non Aktif', $idBudgetingProjectProgres]
            );

            if ($status) {
                $pesan = 'Proses Non Aktif Budgeting Project Berhasil';
            } else {
                $pesan = 'Proses Non Aktif Budgeting Project Gagal';
            }
            $more = [];
