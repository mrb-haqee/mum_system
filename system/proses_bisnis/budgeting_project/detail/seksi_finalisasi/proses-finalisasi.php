    ...
    try {
        ...
        if ($flag === 'finalisasi') {
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE budgeting_project SET statusBudgetingProject = ? WHERE kodeBudgetingProject = ?',
                ['Aktif', $kodeBudgetingProject]
            );

            if ($status) {
                updateNomorUrut($db, 'budgeting_project');
                $pesan = 'Proses Finalisasi Budgeting Project Berhasil';
            } else {
                $pesan = 'Proses Finalisasi Budgeting Project Gagal';
            }
        } else {
            $status = false;
