...
function dataDaftarBiaya() {
	const periode = $("#periode").val();

	$.ajax({
		url: "daftar-biaya.php",
		type: "post",
		data: {
			periode,
		},
		...
		success: function (data, status) {
			$("#boxDaftarBiaya").html(data);
			$(".overlay").hide();
		},
	});
}

function getFormBiaya(idBiaya = "") {
...
function getFormBiayaDetail(idBiayaDetail = "") {
	const flag = $("#flag").val();
...
function deleteBiayaDetail(idBiayaDetail, kodeBiaya, token) {
	$.ajax({
		url: "proses-biaya.php",
		type: "post",
		data: {
