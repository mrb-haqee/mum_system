        ...
        if ($flag === 'delete') {
            $dataKode = statementWrapper(
                ...
                "SELECT
                    ...
                    kodeFile
                FROM
                    biaya INNER JOIN uploaded_file ON biaya.kodeBiaya = uploaded_file.noForm
                WHERE
                    idBiaya = ?
                ",
                [$idBiaya]
            ...
            $status = statementWrapper(
                DML_DELETE,
                'DELETE FROM biaya WHERE idBiaya = ?',
                [$idBiaya]
            );
            $statusDetail = statementWrapper(
                DML_DELETE,
                'DELETE FROM biaya_detail WHERE kodeBiaya = ?',
                [$dataKode['kodeBiaya']]
            );
            ...
            $status = statementWrapper(
                DML_DELETE,
                'DELETE FROM biaya_detail WHERE idBiayaDetail = ?',
                [$idBiayaDetail]
            );
            ...
            if ($status) {
                updateNomorUrut($db, 'biaya', $idUserAsli);
