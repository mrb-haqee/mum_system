    ...
    try {
        ...
        if ($flag === 'finalisasi') {
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE account SET statusAccount = ? WHERE kodeAccount = ?',
                ['Aktif', $kodeAccount]
            );

            if ($status) {
                updateNomorUrut($db, 'account');
                $pesan = 'Proses Finalisasi Account Berhasil';
            } else {
                $pesan = 'Proses Finalisasi Account Gagal';
            }
        } else {
            $status = false;
