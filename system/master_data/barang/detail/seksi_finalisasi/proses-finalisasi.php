    ...
    try {
        ...
        if ($flag === 'finalisasi') {
            $status = statementWrapper(
                DML_DELETE,
                'UPDATE barang SET statusBarang = ? WHERE kodeBarang = ?',
                ['Aktif', $kodeBarang]
            );

            if ($status) {
                updateNomorUrut($db, 'inventory_Barang');
                $pesan = 'Proses Finalisasi Barang Berhasil';
            } else {
                $pesan = 'Proses Finalisasi Barang Gagal';
            }
        } else {
            $status = false;
