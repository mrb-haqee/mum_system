        ...
        if ($flag == 'aktivasiHak') {
            ...
            if ($statusMenu === 'Active') {
                ...
                if ($type === 'widget') {
                    ...
                    if (preg_match('/__ALL__\d+__/', $idItem)) {
                        $idMenu = preg_replace('/\D/', '', $idItem);

                        $dataFormatAksi = statementWrapper(
                            DML_SELECT_ALL,
                            'SELECT 
                                menu_sub.formatAksi, 
                                menu_sub.idSubMenu 
                            FROM 
                                ...
                                LEFT JOIN (
                                    ...
                                    WHERE
                                        idUser = ?
                                ) data_detail ON menu_sub.idSubMenu = data_detail.idSubMenu 
                            WHERE 
                                data_detail.idUserDetail IS NULL
                                AND menu_sub.idMenu = ?',
                            [$idUserAccount, $idMenu],
                        );

                        $listFormatAkses = array_combine(array_column($dataFormatAksi, 'idSubMenu'), array_column($dataFormatAksi, 'formatAksi'));

                        $insertQM = [];
                        $execute = [];

                        foreach ($listFormatAkses as $idSubMenu => $formatAkses) {
                            $json_decoded = json_decode($formatAkses, true);
                            $hakAkses = array_combine($json_decoded, array_fill(0, count($json_decoded), 'Active'));

                            $json_encoded = json_encode($hakAkses);
                            $insertQM[] = "(?,?,?)";

                            $execute = array_merge($execute, [$idUserAccount, $idSubMenu, $json_encoded]);
                        }

                        $insertList = join(', ', $insertQM);
