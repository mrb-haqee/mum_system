    ...
    try {
        if ($flag == 'delete') {
            $status = statementWrapper(
                DML_UPDATE,
                "UPDATE petty_cash SET statusPettyCash = ? WHERE idPettyCash = ?",
                ['Non Aktif', $idPettyCash]
            );

            if ($status) {
                $pesan = 'Proses Non Aktif Petty Cash Berhasil';
            } else {
                $pesan = 'Proses Non Aktif Petty Cash Gagal';
            }
        } else {
            $status = false;
