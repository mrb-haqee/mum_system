    ...
    try {
        ...
        if ($flag === 'finalisasi') {
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE petty_cash SET statusPettyCash = ? WHERE kodePettyCash = ?',
                ['Aktif', $kodePettyCash]
            );

            if ($status) {
                updateNomorUrut($db, 'petty_cash');
                $pesan = 'Proses Finalisasi Petty Cash Berhasil';
            } else {
                $pesan = 'Proses Finalisasi Petty Cash Gagal';
            }
        } else {
            $status = false;
