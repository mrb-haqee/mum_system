    ...
    try {
        ...
        if ($flag === 'delete') {
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE pegawai SET statusPegawai = ? WHERE kodePegawai = ?',
                ['Non Aktif', $kodePegawai]
            );

            $statusDetail = statementWrapper(
                DML_UPDATE,
                'UPDATE pegawai_jaspel SET statusPegawaiJaspel = ? WHERE kodePegawai = ?',
                ['Non Aktif', $kodePegawai]
            );

            if ($status) {
                $pesan = 'Proses Non Aktif Pegawai Berhasil';
            } else {
                $pesan = 'Proses Non Aktif Pegawai Gagal';
            }
        } else {
            $status = false;
