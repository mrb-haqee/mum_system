...
/**
 * Parameter requirements = ["kodeRM", "kodeAntrian", "jenisSurat" => 'Surat Rapid Test Antigen']
 */
function viewValidatorSuratRapidTestAntigen(array $requirements, array $params)
{
    foreach ($requirements as $requirement) {
        if (!in_array($requirement, array_keys($params))) return false;
    }

    if ($params['jenisSurat'] !== 'Surat Rapid Test Antigen') return false;
    ...
    $qr_filename = escapeFilename("QRVALIDATOR_{$params['kodeAntrian']}_Surat_Rapid_Test_Antigen");
?>
    <div style="width: 100%;" class="text-center mb-10 mb-lg-20">
        ...
        <h1 class="font-weight-bolder">CONFIDENTIAL RAPID TEST ANTIGEN</h1>
    </div>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-7 text-lg-left text-md-left text-sm-center text-center mb-4" style="font-size: 1.2rem;">
                <p>Confidential Rapid Test Antigen untuk pasien :</p>
                <span class="font-size-h1 font-weight-bolder"><?= $dataPasien['namaPasien']; ?></span>
                <p class="mt-5 mb-1">Diterbitkan di <strong><u><?= $dataAntrian['namaKlinik']; ?></u></strong></p>
                <p>pada tanggal <strong><u><?= ubahTanggalIndo($dataAntrian['tanggalPendaftaran']); ?></u></strong> oleh <strong><u><?= $dataAntrian['namaDokter']; ?></u></strong></p>
            </div>
        </div>
    </div>
<?php
