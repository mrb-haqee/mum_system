...
?>
    ...
    <select class="form-control selectpicker" id="idBank" name="idBank" data-live-search="true">
        <option value="">Pilih Tujuan</option>
        <?php
        $config = [
            'Bank' => statementWrapper(
                DML_SELECT_ALL,
                "SELECT CONCAT(vendor, ' / ', atasNama) as nama, idTujuanTransfer as idBank FROM tujuan_transfer WHERE statusTujuanTransfer = ?",
                ['Aktif']
            ),
            'Petty Cash' => statementWrapper(
                DML_SELECT_ALL,
                'SELECT namaPettyCash as nama, idPettyCash as idBank FROM petty_cash WHERE statusPettyCash = ?',
                ['Aktif']
            )
        ];
        ...
        ?>
                <option value="<?= $row['idBank'] ?>" <?= $selected ?>>
                    <?= $row['nama'] ?>
                </option>
        <?php
            }
