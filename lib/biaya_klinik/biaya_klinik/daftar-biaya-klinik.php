...
if (!$dataCekUser || !$dataCekMenu || !validateIP($_SESSION['IP_ADDR'])) {
    ...
    $debet['Transfer Penjualan Tunai'] = statementWrapper(
        DML_SELECT,
        "SELECT
        ...
        WHERE
            ...
            AND tipe = ?
        ",
        [$tanggalAwal, $idKlinikAsal, 'Petty Cash']
    );
    ...
    $debet['Pemasukan Lain'] = statementWrapper(
        ...
        WHERE
            ...
            AND petty_cash.tipe = ?
            AND petty_cash.idKlinik = ?
            AND pemasukan_pengeluaran_lain.jenisRekening = ?
        ",
        [$tanggalAwal, 'Pemasukan Lain', 'Petty Cash', $idKlinikAsal, 'Petty Cash']
    );

    $kredit['Biaya Klinik'] = statementWrapper(
        DML_SELECT,
        "SELECT
            SUM(grandTotal) as kredit
        FROM
            biaya_klinik
        WHERE
            (tglBiaya < ?)
    ...
    $kredit['Pengeluaran Lain'] = statementWrapper(
        ...
        WHERE
            ...
            AND pemasukan_pengeluaran_lain.jenisRekening = ?
        ",
        [$tanggalAwal, 'Pengeluaran Lain', 'Petty Cash', $idKlinikAsal, 'Petty Cash']
    );
    ...
    $data = statementWrapper(
        ...
        FROM
            (
                (
                    SELECT
                        idTransferPenjualanTunai as id,
                        tglTransfer as tanggal,
                        'Transfer Petty Cash' as uraian,
                        nominal as debet,
                        0 as kredit,
                        'tambah' as jenis,
                ...
                (
                    SELECT
                        biaya_klinik.idBiayaKlinik as id,
                        biaya_klinik.tglBiaya as tanggal,
                        CONCAT(kode_akunting.namaAkun, ' / ', biaya_klinik_detail.namaItem) as uraian,
                        0 as debet,
                        biaya_klinik.grandTotal as kredit,
                        'kurangBiaya' as jenis,
                        biaya_klinik.timeStamp as timestamp
                    FROM
                        biaya_klinik 
                        INNER JOIN biaya_klinik_detail ON biaya_klinik_detail.kodeBiaya = biaya_klinik.kodeBiaya
                        INNER JOIN kode_akunting ON biaya_klinik.kodeAkun = kode_akunting.kodeAkun
                    WHERE
                        (biaya_klinik.tglBiaya BETWEEN ? AND ?)
                        AND biaya_klinik.idKlinik = ?
                    GROUP BY biaya_klinik_detail.kodeBiaya
                )
                UNION ALL
                (
                    SELECT
                    ...
                    WHERE
                        ...
                        AND petty_cash.tipe = 'Petty Cash'
                        AND petty_cash.idKlinik = ?
        ...
        ",
        [
            $tanggalAwal, $tanggalAkhir, $idKlinikAsal, 'Petty Cash',
            $tanggalAwal, $tanggalAkhir, $idKlinikAsal,
            $tanggalAwal, $tanggalAkhir, $idKlinikAsal,
            $tanggalAwal, $tanggalAkhir, $idKlinikAsal,
        ]
    );
                    ...
                    <td>
                        ...
                        ?>
                            ...
                            <div class="dropdown-menu menu-aksi" aria-labelledby="dropdownMenuButton">
                                <button type="button" class="btn btn-warning btn-sm tombol-dropdown" onclick="getFormBiayaKlinik('<?= $row['id'] ?>')">
