...
if (!$dataCekUser || !$dataCekMenu || !validateIP($_SESSION['IP_ADDR']) || !$tokenValid) {
    ...
    try {
        ...
        if ($flag === 'update') {
            ...
            $nominal = ubahToInt($nominal ?? 0);
            $idHotel = $idHotel ?? NULL;

            if (!is_null($idHotel)) {
                $nama = statementWrapper(
                    DML_SELECT,
                    'SELECT namaHotel FROM hotel WHERE idHotel = ?',
                    [$idHotel]
                )['namaHotel'];
            }
            ...
            if ($status) {
                $pesan = 'Proses Update Fee Oncall Berhasil';
            } else {
                $pesan = 'Proses Update Fee Oncall Gagal';
            }
        } else if ($flag === 'tambah') {
            ...
            if ($status) {
                $pesan = 'Proses Tambah Fee Oncall Berhasil';
            } else {
                $pesan = 'Proses Tambah Fee Oncall Gagal';
            }
        } else {
            $status = false;
