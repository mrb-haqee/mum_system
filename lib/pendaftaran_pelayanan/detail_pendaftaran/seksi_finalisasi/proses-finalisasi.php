...
if (!$dataCekUser || !$dataCekMenu || !validateIP($_SESSION['IP_ADDR']) || !$tokenValid) {
    ...
    try {
        ...
        if ($flag === 'finalisasi') {
            ...
            $nomorAntrian = nomorAntrian($db, intval($dataAntrian['idKlinik']))['nomorAntrian'];
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE pasien_antrian SET nomorAntrian = ?, statusAntrian = ? WHERE kodeAntrian = ?',
                [$nomorAntrian, 'Aktif', $kodeAntrian]
            );

            if ($status) {
                updateNomorUrut($db, 'antrian_klinik');

                $pesan = 'Proses Finalisasi Pendaftaran Berhasil';
            } else {
                $pesan = 'Proses Finalisasi Pendaftaran Gagal';
            }
        } else {
            $status = false;
