    ...
    try {
        if ($flag == 'delete') {
            ...
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE pasien_antrian SET statusAntrian = ? WHERE kodeAntrian = ?',
                ['Void', $kodeAntrian]
            );

            $statusPemeriksaan = statementWrapper(
                DML_UPDATE,
                'UPDATE pasien_pemeriksaan_klinik SET statusPasienPemeriksaan = ? WHERE kodeAntrian = ?',
                ['Void', $kodeAntrian]
            );

            if ($status) {
                $pesan = 'Proses Void Pendaftaran Berhasil';
            } else {
                $pesan = 'Proses Void Pendaftaran Gagal';
            }
        } else {
            $status = false;
