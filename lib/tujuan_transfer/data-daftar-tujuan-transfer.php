...
if (!$dataCekUser || !$dataCekMenu || !validateIP($_SESSION['IP_ADDR'])) {
    ...
    $dataTujuanTransfer = statementWrapper(
        DML_SELECT_ALL,
        "SELECT * FROM tujuan_transfer WHERE statusTujuanTransfer = ? {$parameter['nama']}",
        $execute
    );
?>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <div class="dropdown-menu menu-aksi" aria-labelledby="dropdownMenuButton">
                            ...
                            <button type="button" class="btn btn-danger btn-sm tombol-dropdown-last" onclick="konfirmasiBatalTujuanTransfer('<?= $row['idTujuanTransfer'] ?>', '<?= $tokenCSRF ?>')">
                                <i class="fa fa-trash"></i> <strong>HAPUS</strong>
                            </button>
                        </div>
                    </td>
                    <td>
                        <span class="d-block font-weight-bold"><?= $row['noReferensi'] ?></span>
