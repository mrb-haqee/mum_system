    ...
    try {
        ...
        if ($flag === 'finalisasi') {
            $status = statementWrapper(
                DML_UPDATE,
                'UPDATE tujuan_transfer SET statusTujuanTransfer = ? WHERE kodeTujuanTransfer = ?',
                ['Aktif', $kodeTujuanTransfer]
            );

            if ($status) {
                updateNomorUrut($db, 'tujuan_transfer');
                $pesan = 'Proses Finalisasi Tujuan Transfer Berhasil';
            } else {
                $pesan = 'Proses Finalisasi Tujuan Transfer Gagal';
            }
        } else {
            $status = false;
