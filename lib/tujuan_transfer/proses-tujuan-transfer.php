    ...
    try {
        if ($flag == 'delete') {
            $status = statementWrapper(
                DML_UPDATE,
                "UPDATE tujuan_transfer SET statusTujuanTransfer = ? WHERE idTujuanTransfer = ?",
                ['Non Aktif', $idTujuanTransfer]
            );

            if ($status) {
                $pesan = 'Proses Non Aktif Tujuan Transfer Berhasil';
            } else {
                $pesan = 'Proses Non Aktif Tujuan Transfer Gagal';
            }
        } else {
            $status = false;
